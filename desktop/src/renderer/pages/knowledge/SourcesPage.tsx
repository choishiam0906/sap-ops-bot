        ...
        <div className="sources-grid">
          <section className="sources-panel">
            ...
            <div className="sources-form">
              <label className="sources-label">
                서버 이름
                <input className="sources-input" value={mcpName} onChange={(event) => setMcpName(event.target.value)} placeholder="예: sap-docs-server" />
              </label>
              <label className="sources-label">
                실행 명령어
                <input className="sources-input" value={mcpCommand} onChange={(event) => setMcpCommand(event.target.value)} placeholder="예: node, npx, python" />
              </label>
              <label className="sources-label">
                인수 (공백 구분)
                <input className="sources-input" value={mcpArgs} onChange={(event) => setMcpArgs(event.target.value)} placeholder="예: server.js --port 3100" />
              </label>
              <div className="sources-form-actions">
                <Button type="button" onClick={handleMcpConnect} loading={isConnecting} disabled={!mcpCommand.trim() || !mcpName.trim()}>
            ...
            <div className="sources-card-list">
              {connectedServers.length === 0 && <div className="sources-empty">연결된 MCP 서버가 없습니다.</div>}
              {connectedServers.map((serverName) => (
                <article key={serverName} className={`source-card ${selectedMcpServer === serverName ? 'active' : ''}`}>
                  <div className="source-card-header">
                    <div>
                  ...
                  <div className="source-card-actions">
                    <Button variant="secondary" size="sm" type="button" onClick={() => setSelectedMcpServer(serverName)}>리소스 보기</Button>
                    <Button variant="ghost" size="sm" type="button" onClick={() => handleMcpDisconnect(serverName)}>
                ...
                <div className="sources-card-list">
                  {mcpSources.map((source) => (
                    <article key={source.id} className="source-card">
                      ...
                      <div className="source-card-actions">
                        <Button variant="ghost" size="sm" type="button" onClick={() => handleMcpSync(source)} loading={reindexingId === source.id}>
                          <RefreshCw size={14} aria-hidden="true" />
                          재동기화
                        </Button>
                      </div>
          ...
          <section className="sources-panel">
            <div className="sources-panel-header">
              <div>
                ...
                <h2>{selectedMcpServer ? `${selectedMcpServer} 리소스` : 'MCP 리소스'}</h2>
              </div>
            ...
            {selectedMcpServer && (
              <div className="sources-form">
                <label className="sources-label">
                  Source 등록 이름
                  <input className="sources-input" value={mcpSourceTitle} onChange={(event) => setMcpSourceTitle(event.target.value)} placeholder={`예: ${selectedMcpServer} 문서`} />
                </label>
                <label className="sources-label">
                  기본 분류
                  <select className="sources-select" value={mcpClassification} onChange={(event) => setMcpClassification(event.target.value as VaultClassification)}>
                    <option value="confidential">confidential</option>
                    <option value="reference">reference</option>
                  </select>
                </label>
                <div className="sources-form-actions">
                  <Button type="button" onClick={() => handleMcpAddSource(selectedMcpServer)} loading={isAddingMcpSource}>
            ...
            <div className="sources-card-list">
              {!selectedMcpServer && <div className="sources-empty">왼쪽에서 연결된 MCP 서버를 선택하면 리소스를 확인할 수 있습니다.</div>}
              ...
              {selectedMcpServer && !isLoadingResources && mcpResources.length === 0 && <div className="sources-empty">이 서버에 노출된 리소스가 없습니다.</div>}
              {mcpResources.map((resource) => (
                <article key={resource.uri} className="source-doc-card">
                  ...
                  {resource.description && <p className="source-doc-excerpt">{resource.description}</p>}
                </article>
              ))}
            </div>
          </section>
